const box_design_classify = $("#box-design-classify")
...
const input_design_key_classify = $(".input-design-key-classify")
const input_design_key_size = $(".input-design-key-size")
...
var createBoxSize = (box_classify,kich_co = null)=>{
    let classify_size_rename = box_classify.find(".classify-size-rename")
    let doi_ten_kich_co = classify_size_rename.val() != "" ? classify_size_rename.val() : (input_design_key_size.val() ?? "Kích cở");    
    let s =`<div class="box-size-details">
            <input type="text" class=" input-create-info margin-bottom-5 input-size-name" placeholder="Tên ${doi_ten_kich_co}">
    ...
    if(kich_co!=null)
    {
        input_size_name.val(kich_co.ten_kich_co)
        input_size_quantity.val(kich_co.so_luong_ton) 
        if(kich_co.gia_hien_tai)
            input_size_price.val(kich_co.gia_hien_tai);  
    }
...
var showImageClassify = (fileName, fileUrl,data_image,input_choose_image_classify = null, file = null)=>{
    if(images[fileName])
    {
        if(input_choose_image_classify)
            input_choose_image_classify.val("");
        return handleCreateToast("info","ảnh này đã tải lên, vui lòng chọn ảnh khác");       
    }                                                
    let img = $(`<div class="image classify" id="${fileName}" style="background: url(${fileUrl}); background-size:cover " title="Click vào đây để xóa cái ảnh này">
...
var showFileImageClassify = (files,input_choose_image_classify,title_choose_image,data_image)=>{
    ...
    if (validExtensions.includes(fileType)) {
        ...
        fileReader.onload = function () { 
            ...
            showImageClassify(file.name,fileUrl,data_image,input_choose_image_classify,file)
            // if(images[file.name])
            //     return handleCreateToast("info","ảnh này đã tải lên, vui lòng chọn ảnh khác");                                                        
...
var createBoxClassify = (i,phan_loai = null,linkUrlImage=null)=>{
    let doi_ten_phan_loai = input_design_key_classify.val() ?? "Phân loại"
    let doi_ten_kich_co = input_design_key_size.val() ?? "Kích cở";    
    let s=`<div class="box-classify">
                <input type="text" class="input-classify-name input-create-info" placeholder="Tên ${doi_ten_phan_loai}...">
                <span class="message-classify-name" style="display:none"><span title="Tên ${doi_ten_phan_loai} không được để trống">*</span></span>
                <div class="item-remove-box-classify" title="click vào đây để xóa cái ${doi_ten_phan_loai} này">
                ...
                <div class="input-check-design-size">
                    <label for="input-checkbox-design-size-${i}">Bật "<span class="size-rename">${doi_ten_kich_co}</span>"</label>
                    <input type="checkbox" class="input-checkbox-design-size" id="input-checkbox-design-size-${i}">
                </div>                
                <div class="box-classify-details">
                    ...
                    <div class="box-design-sizes"> 
                        <div class="line-margin-left-right-20">
                            <div class="line">
                                <span class="item-title-design-size">"<input type="text" class="classify-size-rename input-create-info" title="Khóa '${doi_ten_kich_co}' tùy chỉnh" placeholder="Khóa '${doi_ten_kich_co}' tùy chỉnh">":</span>                                
                                <span class="item-count-size" title="Số lượng ${doi_ten_kich_co}" >(<span class="count-size">0</span>)</span>
                                <button class="btn btn-outline-dark btn-add-size">
                                    ...
                                    "<span class="title-size-rename">${doi_ten_kich_co}</span>" mới
                                </button>
                            </div>
                        </div>                                                                                                                        
    ...
    let input_classify_name = box_classify.find(".input-classify-name")
    input_classify_name.on("blur",function(){
    ...
    var count_size = box_classify.find(".count-size"); 
    ...
    if(phan_loai)
    {    
        input_classify_name.val(phan_loai.ten_phan_loai)
        if(phan_loai.gia_hien_tai)
            input_classify_price_value.val(phan_loai.gia_hien_tai)  
        if(phan_loai.anh_phan_loai)
            showImageClassify(phan_loai.anh_phan_loai,linkUrlImage+phan_loai.anh_phan_loai,data_image)     
        if(phan_loai.kich_co_phan_loais)
        {
            if(phan_loai.doi_ten_kich_co)
            {
                classify_size_rename.val(phan_loai.doi_ten_kich_co)
                box_classify.find(".item-count-size").attr("title","Số lượng "+phan_loai.doi_ten_kich_co)
                title_size_rename.text(phan_loai.doi_ten_kich_co)
            }
            input_checkbox_design_size.attr("checked",true)
            phan_loai.kich_co_phan_loais.forEach(kich_co=>{
                box_size.append(createBoxSize(box_classify,kich_co));
            })
        }
        else
        {
            input_classify_quantity_value.val(phan_loai.so_luong_ton)
            input_classify_quantity.slideDown()
            box_size.append(createBoxSize(box_classify));
            box_design_sizes.slideUp(); 
            input_classify_quantity.slideDown();
            input_classify_price.addClass("col-md-6 col-lg-6");
        }
    }
...
input_design_key_classify.blur(function(){    
    ...
    $(".input-classify-name").each(function(){
        ...
        $(".message-classify-name").each(function(){
            $(this).attr("title",`Tên ${key_classify} không được để trống`)
        })
        $(".item-remove-box-classify").each(function(){
            $(this).attr("title",`click vào đây để xóa cái ${key_classify} này`)
        })
    })    
});
...
input_design_key_size.blur(function(){
    ...
    {   
        ...
        $(box_classify).find(".item-count-size").attr("title",`Số lượng ${key_size}`)
...
var convertCoverImage = (img,box_cover_image,file)=>{
    ...
    setCoverImage(img,box_cover_image,file)
}


var setCoverImage = (img,box_cover_image,file)=>{
    delete images[img.attr("id")]
...
var createEventConvertCoverImage = (cover_image,img,box_cover_image,file)=>{
    ...
    cover_image.click(function(){  
        convertCoverImage(img,box_cover_image,file)        
    })
}

var showImage = (url,imageName,file=null,anh_bia=null)=>{    
    count_image++;                             
    let img = $(`<div class="image image-main" id="${imageName}" style="background: url(${url}); background-size:cover ">
    ...
    img.data("file-name",imageName);  
    let box_cover_image = img.find(".box-cover-image");
    let cover_image = $(`<div class="cover-image">                                            
                            </div>`)
    if(((anh_bia!=null && anh_bia == imageName ) || (anh_bia == null && count_image == 1)) && $("#cover-image")[0] == undefined)
    {                
        img.attr("id","cover-image");                    
    ...
    else
    {
        ...
        createEventConvertCoverImage(cover_image,img,box_cover_image,file ?? imageName)
    }                                                                   
    ...
    img.find(".item-remove-image").click(function(){
        ...
        delete images[id];
        img.remove();
        input_choose_image.val("")
        count_image--;
        if( id == "cover-image" && count_image > 0)                    
        {            
            img_cover = $($(".image-main")[0]);
            console.log(img_cover)
            setCoverImage(img_cover,img_cover.find(".box-cover-image"),images[img_cover.attr("id")])
            console.log(images);        
        }
            //$($(".convert-cover-image")[0]).click();  
    })              
    list_data_image.append(img); 
    images[img.attr("id")] = file ?? imageName;
...
function showFileImage(file) {            
    ...
    if (validExtensions.includes(fileType)) {
        ...
        fileReader.onload = function () {  
            ...
            {
                const fileUrl = fileReader.result; 
                showImage(fileUrl,file.name,file);
                // count_image++;                                               
...
var showVideo = (videoURL,videoName, file = null)=>{
    ...
    box_video.data("video-name",videoName) 
...
function showFileVideo(file)
{
    ...
    if (validVideoExtensions.includes(fileType)) {
        ...
        showVideo(videoURL,file.name,file);
        // let box_video = $(`<div id="box-video">
        //                         <video class="video" id="video-show" controls>
        //                             <source id="video-show-source" src="${videoURL}" type="video/mp4" autoplay>
        //                         </video>
...
var createProductInfoDetails = (thuoc_tinh=null,gia_tri=null)=>{
    ...
    if(thuoc_tinh && gia_tri)
    {        
        input_create_info_details_key.val(thuoc_tinh)
        input_create_info_details_value.val(gia_tri)
    }
...
var addItemProductInfoDetails = (thuoc_tinh=null,gia_tri=null) =>{
    $("#box-create-product-details").append(createProductInfoDetails(thuoc_tinh,gia_tri));
...
var buildValidateUpdateProductEvent = (message,url,method)=>{
    $("#btn-submit-add-product").click(function(){    
        ...
        if(chk_validate.indexOf(false)==-1)        
            showMessage("Thông báo",message,function(){     
                // console.log(images)
                ...
                //     }
                // }
                let file_name = images["cover-image"].name ?? images["cover-image"];
                ...
                fdata.append("san_pham", JSON.stringify(san_pham));                        
                ...
                $.ajax({
                    ...
                    success: function (res) {  
                        ...
                        if(res.success)
                        {
                            handleCreateToast("success","Cập nhật sản phẩm thành công");
...
var loadProduct = (san_pham_id)=>{
    GET(BASE_URL_API+PREFIX_PRODUCT+san_pham_id,(data)=>{
        console.log(data)
        san_pham = data.sanpham;
        input_create_product_name.val(san_pham.ten_san_pham)
        input_create_price_all.val(san_pham.gia_hien_tai ?? "")        
        let url = `${URL_HOST}uploads/${JsonParseStr(san_pham.cua_hang_id)}/${JsonParseStr(san_pham._id)}/`
        if(san_pham.phan_loais)
        {
            if(san_pham.doi_ten_phan_loai)
            {
                input_design_key_classify.val(san_pham.doi_ten_phan_loai)
                if(san_pham.doi_ten_kich_co)
                    input_design_key_size.val(san_pham.doi_ten_kich_co)
            }
            check_create_classify.attr("checked",true);
            san_pham.phan_loais.forEach(phan_loai=>{
                box_design_classify.append(createBoxClassify(i++,phan_loai,url));
            })
        }
        else
        {
            input_create_quantity_all.val(san_pham.so_luong_ton)
            $("#item-create-quantity").slideDown();
            $("#box-create-classify ").slideUp();
            box_design_classify.append(createBoxClassify(i++))
        }
        for (const key in san_pham.thong_tin_chi_tiet) {
            if (Object.hasOwnProperty.call(san_pham.thong_tin_chi_tiet, key)) {
                addItemProductInfoDetails(key,san_pham.thong_tin_chi_tiet[key])                
            }
        }
        if(san_pham.anh_san_phams)
            san_pham.anh_san_phams.forEach(anh_san_pham => {
                let urlImage = url + anh_san_pham
                showImage(urlImage,anh_san_pham,null,san_pham.anh_bia); 
            });
        if(san_pham.video)
            showVideo(url+san_pham.video,san_pham.video);
        if(san_pham.giam_gia)
        {
            check_create_discount.attr("checked",true);
            disabledInputDiscount(false)
            input_create_product_discount_start_day.val(ConvertDateTimeToString(san_pham.ngay_bat_dau))
            input_create_product_discount_end_day.val(ConvertDateTimeToString(san_pham.ngay_ket_thuc))
            input_create_product_discount_ratio.val(san_pham.giam_gia*100)
            input_create_product_discount_limited_quantity.val(san_pham.so_luong_gioi_han)
        }
        else
            disabledInputDiscount(true)
        buildValidateUpdateProductEvent("Xác nhận cập nhật sản phẩm?",BASE_URL_API+PREFIX_STORE+PREFIX_PRODUCT+san_pham_id,"POST");
    });
}
var loadEventCreateProduct = ()=>{
    box_design_classify.append(createBoxClassify(i++)) 
    check_create_classify.attr("checked",true);   
    addItemProductInfoDetails();
    check_create_discount.attr("checked",true);
    buildValidateUpdateProductEvent("Xác nhận tạo sản phẩm?",BASE_URL_API+PREFIX_STORE+PREFIX_PRODUCT+PREFIX_PRODUCT_CREATE,"POST");
}
